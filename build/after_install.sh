...
#    Chromium aborts. The generated deb never sets this bit.
...
#    absolute binary path so clicking the app icon opens Phantom.
#
# NOTE: electron-builder substitutes dollar-brace macros in hook scripts,
# so this file must NEVER contain them (use plain $VAR — bash needs no braces
# here). That was the "Macro BIN is not defined" CI failure.
set -e

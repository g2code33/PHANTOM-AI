...
| **12 · Health Brain** | Independent Health entity (own identity/prompt/model/tools/memory); encrypted Health Memory Vault (Fernet, chmod-600 key, strict separation from general memory); daily health management + today overview; medication/appointment/goal tracking; measurement trends (no diagnoses); symptom tracking; red-flag safety layer (emergency/warning thresholds + structured output the model cannot override); privacy controls (no sharing default, non-sensitive briefing, audit); personalized routines → heartbeat schedules; study cooperation; Health Center UI; disable/re-enable | `tests/test_health.py` (15), `tests/test_api.py` (health endpoints), live smoke (4 brains) |
| **13 · Jarvis transformation** | App renamed **Phantom** (0.2.0; productName/appId/deb/exe/AppImage/APK names, window title); voice-first presence UI (`ui/voice.js` real-time engine: IDLE/LISTENING/THINKING/SPEAKING/INTERRUPTED/EXECUTING/VERIFYING/ERROR/DISCONNECTED, streaming STT (browser/Deepgram via server-minted ephemeral token), streaming TTS with speak queue, energy VAD, barge-in, reconnect); proactive speech via heartbeat (`voice.speak` WS events, cooldown, quiet-by-default); kill switch stops voice+mic (`voice.stop`); full 31-capability specialist brain catalogue seeded; provider-status caching; first-run onboarding; local/online mode indicator | `tests/test_voice.py` (8), `tests/test_config.py` (2), `tests/test_api.py` (brains superset), full suite 105 passed |
